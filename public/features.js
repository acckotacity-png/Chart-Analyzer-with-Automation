...
function applyQuote(stock,data){stock.price=data.quote.price;stock.change=data.quote.change;stock.changePercent=data.quote.previousClose>0&&Number.isFinite(stock.change)?(100*stock.change/data.quote.previousClose).toFixed(2):null;stock.quoteTime=data.quote.lastTradeAt;if(candleSeries&&Number.isFinite(stock.price)){if(livePriceLine){try{candleSeries.removePriceLine(livePriceLine);}catch{}}livePriceLine=candleSeries.createPriceLine({price:stock.price,color:'rgb(0,229,255)',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'Live LTP'});}const price=el('selectedStockPrice');if(price){price.classList.remove('quote-tick');void price.offsetWidth;price.classList.add('quote-tick');price.textContent=money(stock.price);}const tickTime=new Date(data.quote.lastTradeAt).toLocaleTimeString('en-IN',{timeZone:'Asia/Kolkata',hour:'2-digit',minute:'2-digit',second:'2-digit'});report('liveQuoteTime','Live tick received · '+tickTime+' IST');const change=el('selectedStockChange');if(change)change.style.color='var(--green)';report('selectedStockChange',Number.isFinite(stock.change)?money(stock.change)+(stock.changePercent!==null?' ('+stock.changePercent+'%)':''):'Change unavailable');renderSharesList();populateQuickStockDropdown();}
setInterval(async()=>{
