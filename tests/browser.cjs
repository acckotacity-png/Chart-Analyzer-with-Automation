...
try{
 ...
 await page.addInitScript(()=>{
  ...
  window.__client={auth:{getSession:async()=>({data:{session:{access_token:'test'}}}),getUser:async()=>({data:{user}}),signOut:async()=>({})},from:t=>new Query(t),rpc:async(name,args)=>{window.__calls.push({name,args});return {data:name==='has_active_access'?true:null,error:null};}};
 });
